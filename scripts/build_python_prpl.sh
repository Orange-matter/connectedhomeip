...
OUTPUT_ROOT="$CHIP_ROOT/prpl/python_lib"
...
SYSROOT_PRPL=${PW_PROJECT_ROOT}/third_party/prpl-sysroot
PKG_CONFIG_SYSROOT_DIR=${SYSROOT_PRPL}
PKG_CONFIG_LIBDIR=${SYSROOT_PRPL}/usr/lib/pkgconfig:${SYSROOT_PRPL}/usr/share/pkgconfig
GN_PRPL=("treat_warnings_as_errors=false prpl_sysroot=\"$SYSROOT_PRPL\" target_build=\"prpl\" custom_toolchain=\"../..//build/toolchain/custom\" ")

gn --root="$CHIP_ROOT" gen "$OUTPUT_ROOT" --args="$GN_PRPL $tracing_options chip_detail_logging=$chip_detail_logging enable_pylib=$enable_pybindings enable_rtti=$enable_pybindings chip_project_config_include_dirs=[\"//config/python\"] $chip_mdns_arg $chip_case_retry_arg $pregen_dir_arg"
