...
static const char * TAG = "extender-app-task";
...
void AppTask::ResetFactoryActionEventHandler(AppEvent * aEvent)
{
    ESP_LOGW(TAG, "Reset Factory");
...
void AppTask::ButtonRisingCallback()
{
    ...
    button_event.mHandler = AppTask::RssiActionEventHandler;
...
void AppTask::ButtonTimerCallback(TimerHandle_t xTimer)
{
    if(!AppButton.RssiState())
    {
        ...
        AppEvent button_event;
        button_event.Type     = AppEvent::kEventType_Button;
        button_event.mHandler = AppTask::ResetFactoryActionEventHandler;
        sAppTask.PostEvent(&button_event);
    }
}
